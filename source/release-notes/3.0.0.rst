v3.0.0
======

V3.0.0 brings about a large change compared to the V2 releases.
These changes range from just basic UI updates to full-on changes to how
things function with servers. These changes are based on feedback and a 
better understanding of how things are used. V2 was a drastic change from
it's previous version, and V3 is no different.

Below is a list of the larger changes:

- UI overhauled across the board. The changes were made based on new features 
  being available 
- Community templates are now stored in GitHub and pulled from there when used,
  instead of being copied locally
- Host environment is ran using unshare, to help improve server security. This
  can be disabled on the server or daemon level. It is recommended to only disable
  it on a server if the server requires accessing files from another server.
- Telnet and RCON is now supported for game servers that require it for commands
- Windows support is now fully removed
- Stats for Java-based servers are now shown, providing better visibility into the server
- DepotDownloader will download the latest version of the tool
- SFTP usernames have been updated to use characters that should not cause issues with
  other clients or tools
- NeoForge is now an available operator
- Admins can SFTP to servers without being explicitly defined
- By default, if the panel is installed using Docker, only Docker will be available for
  servers
- Operations in templates now support conditions
- A basic backup system. This just makes a compressed copy of the server and stores it on
  the disk in a specified path in the config. It is recommended to have this path be a
  network drive/folder so that backups are not lost if the machine does fail.


This contains the full changelog of the 3.0.0 release cycle.

v3.0.0 - Release Candidate 15
-----------------------------

This is a significant update compared to the previous releases.

This release candidate contains a "breaking" change in regards to how
servers are launched in order to reduce "concerns" about servers and
their access to files using a tool called unshare.

**Note, this does NOT make it a replacement for Docker**

Servers which use the "host" environment will now run in an unshare
environment structure. This will attempt to isolate the server from the
host files, however note that this is NOT perfect. It cannot fully
isolate servers, so there are some files that might be accessible that
is not desired. If perfect isolation is desired, use Docker as the environment.

Should a server have issues with unshare, the "disableUnshare" flag may be set
to "true" in the server's environment definition to disable unshare for that
server. Alternatively, should the host machine have issues with unshare, the
system can be entirely disabled by setting the "security.disableUnshare" in
the panel config to "true".

This also removes the Windows build. While they were provided as a "no-support",
it is simply easier to stop providing them.

Changelog
#########
- Change host environment to use unshare by default
- Adjust error message for when CurseForge project does not have 3rd party support enabled
- Add options for setting trusted proxies so logs can have correct IPs
- Fix TOTP field
- Add better error message for when a template cannot be used
- Update CLI lib so the command-line interface has an updated look and feel
- Remove Windows
- Disable special stats when installing servers, resolves issues with FTB installer
- Add recovery tokens for 2FA credentials
- Improve stat chart interactions


v3.0.0 - Release Candidate 13
-----------------------------

Changelog
#########
- Fix issues with CurseForge template not properly resolving variables
- Fix resolvers for Forge and Neo to not fail if folders don't exist
- Fix TOTP to support Enter key and better detection of entered codes
- Fix Chrome adding scrollbars in bad locations


v3.0.0 - Release Candidate 12
-----------------------------

Changelog
#########
- Fix user creation to ensure home directory is managed
- Fix cookie settings to work for non-http usage
- Enable ARM64 RPM builds
- Fix CPU calculation for Docker based on documentation
- Fix disallow host config option to correctly remove the option in the UI
- Fix UI to properly allow changing flags if no data is defined


v3.0.0 - Release Candidate 11
-----------------------------

Changelog
#########
- Expose config to change cookies settings, change secure to false by default
- Force Docker settings to reload when admins edit a server
- Expose test button for emails
- Clean up email system and update to a more supported library
- Expose way to disable host environment when using Docker
- Additional scheduler improvements
- Add stdin to execute a command within a server
- Add debug option to log SFTP commands for when a client fails to properly work
- Update DepotDownloader to support Lancache servers
- Add CLI option to remove 2FA off of an account
- Fix install script to create home


v3.0.0 - Release Candidate 10
-----------------------------

Changelog
#########
- Resolve issue creating new accounts
- Allow editing/deleting of files with non-standard characters through UI


v3.0.0 - Release Candidate 9
----------------------------

Changelog
#########
- Additional fixes to get SFTP working for admins
- Resolve security issue discovered during refactor
- Add unit test to ensure SFTP works
- Remove server name required to be unique
- Correct registration toggle displaying when disabled
- Adjust UI for wider screens
- Adjustments to attempt to auto-resolve volumes for when panel is running in Docker
- Have migrator dump templates which are not valid to a file


v3.0.0 - Release Candidate 7
----------------------------

Changelog
#########
- Update DepotDownloader to resolve Steam download issues
- Correct issues with migrating data
- Update CurseForge runners to support NeoForge-based modpacks
- Use ServerStarter for NeoForge-based servers
- Fix permissions so admins can SFTP into servers they are not explicitly defined in
- Implement a basic backup system
- Correct emails to follow RFC 5322
- Fix issues with editing Docker settings


v3.0.0 - Release Candidate 3
----------------------------

Changelog
#########
- Add display for JCMD stats
- Resolve issues with migrations for Postgres and MySQL
- Fix issue with local node message appearing for non-local nodes


v3.0.0 - Release Candidate 2
----------------------------

Changelog
#########
- Fix console from swallowing messages when using HTTP fallback
- Fix updating nodes
- Fix pipeline to properly set CurseForge key
- Add support for Docker labels
- Fix password validation to now be consistent across CLI and web
- Fix checking for running Docker container to be explicit
- Validate server is not running when deleting the server
- Return specific error when account is not permitted to log in
- Add JCMD support. Allows for getting memory usage from within Java, instead of just what the OS sees.
- Fix server edit to avoid creating a duplicate node


v3.0.0 - Release Candidate 1
----------------------------

Changelog
#########
- Fix CurseForge operation to handle parsing variables.txt with comments
- Add translation key for missing operator


v3.0.0 - Alpha 10
-----------------

Changelog
#########
- Fix the UI and backend for saving docker settings including port bindings


v3.0.0 - Alpha 9
----------------

Changelog
#########
- Fix issues related to migration for MySQL


v3.0.0 - Alpha 8
----------------

Changelog
#########
- Resolve issues with remote nodes and SFTP access


v3.0.0 - Alpha 7
----------------

Changelog
#########

- Fix openvz support for openat with really old kernels
- Add tooltips to icons
- Fix migrations for clients from v2
- Expose better UI for managing admin-level variables
- Add mask to password for email settings


v3.0.0 - Alpha 6
----------------

Changelog
#########

- Fix GORM and it's method of doing updates

v3.0.0 - Alpha 5
----------------

Changelog
#########

- Fix SFTP UI to use URI encoding for the "Open" button
- Have extraction of archives override existing files
- Clean up archive management to use a central codebase
- Fix console endpoint to handle avoid duping lines as much
- Fix file manager to create zips on the root and wait for them to finish


v3.0.0 - Alpha 4
----------------

Changelog
#########

- Fix openat2 checking
- Allow openat2 usage to be controlled via config
- Have install run migrations instead of being done at user creation
- Apply additional fixes that were done on v2


v3.0.0 - Alpha 3
----------------

Changelog
#########

- This fixes the Docker images so now they are properly tagged for arm64 and amd64 using the same tag (pufferpanel/pufferpanel:v3.0.0-alpha.3)


v3.0.0 - Alpha 2
----------------

Changelog
#########

- Fix SFTP for local nodes not using the new naming scheme and using an invalid check
- Correct local nodes to pull their hostname from the config


v3.0.0 - Alpha 1
----------------

This marks the release candidates of v3.0.0. This release has been years in the making.
It comes with significant changes to the backend and frontend to ensure a smoother experience.

The goals of v3 are to improve on things we learned from v2 and update the UI to a more friendly
look and feel. We are still working on improving aspects of the UI, so things may still change as
we go.

The backend has seen a clean up of how things work. The API has been standardized a bit more and
the documentation has been updated to account for this. The concept of the "proxy" no longer is
exposed publicly, but the API will still proxy requests as needed. We also updated our swagger
documentation to try to be more complete. Do note however it may still be missing some information.

Templates have also been updated with the new concept of "conditions". This is a basic "if" 
statement system that is designed to help consolidate our templates and drive better logic about
if an operator should run. Things that the conditions now support are checking if a variable is 
set or if it's a certain value, if a file exist, or if something is in the PATH env. This means 
our templates now can support both environments at the same time, with the conditions driving 
what operators may be used.

Commands now also support the condition concept. A server can have multiple different commands, 
and one is chosen based on these conditions.

We have also moved to using the "any/any" repo with `packagecloud <https://packagecloud.io/pufferpanel/pufferpanel>`_. 
This means that we have a single repository that handles the packages, so the workarounds for 
getting an "unsupported" or a newly released LTS version of an OS should not be needed anymore.

Our Docker image also has been cleaned up. It has instead now been designed to only run the panel 
and a node which creates Docker containers. It will no longer run servers inside of itself. This, 
while useful to create a quick server, caused the image to be bloated and hard to manage. We also 
exposed a new config entry so that servers using Docker can better expose a folder on the host 
that isn't explicitly defined in the image. Previously, it was an explicit path, and changing it 
required changing several things. Now, a single config entry allows hiding this.

We have also exposed options to pull basic JVM stats and query Minecraft servers. While the UI
work to have this displayed is not done as of RC 2, this is now supported. A future version will
show these stats and information.

We also have created a new "tester" designed to keep our templates more consistent and ensure 
they function prior to being released. Generally speaking, the templates now have been at least 
validated to run and not crash immediately. This does mean we have removed several templates that 
either were not functional or were believed to be dead software.

UI Changes
##########
* Improved flow for server creation, steps are a bit more fluid. Install is also prompted after
  creation to help bring this action to the front.
* Server UI will show a game server icon to help quickly identify a server type. This is controlled
  by the "type".
* Templates are now pulled on demand, and only stored if modifications are required. Local templates
  can be managed independently.
* Node page will show some hardware information about the node. Information includes the OS type, 
  cpu architecture, and if the node supports Docker. This page will also show if a node is online.
* Permissions have been stream-lined a little more. Actions are now more explicitly defined as a
  permission and so there are more permissions available. 
* Backup UI added. This is a basic backup with no frills. Backup and restores can only be done with 
  a server being off. Restores will delete ALL files! Backups are compressed, so large servers will 
  need to account for this.
* Numerous bug fixes and code clean-ups

Backend
#######
* Templates can now directly pull from CurseForge. A key is provided by default so servers can be
  created from modpacks from CurseForge. The panel will attempt to set up the server to the best of
  it's ability.
* Forge will attempt to handle both install methods. This means running the installer should permit
  all versions of Forge to work correctly. However, only newer versions of Forge are validated.
* RCON and TELNET are now supported for commands. If a server needs RCON or TELNET to be used for
  commands, a template can specify a "stdin" section to specify additional information.
* Logging should now support rolling over, so tools like logrotate should properly work. This is not
  fully confirmed though.
* Better hooks into systemd. We will now report that we are starting up/shutting down to systemd.
* Docker image names now support variable replacements
* Numerous bug fixes and code clean-ups
